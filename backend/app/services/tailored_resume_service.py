...
def build_tailored_data(profile: Profile, experience: dict[str, Any]) -> ResumeData:
    ...
    for key in ("job2", "job1"):
        ...
        # The extraction writes its own headline per role now, generated by
        # ChatGPT (see _revise_with_chatgpt in experience_service.py) -- it
        # wins over the profile-lookup fallback above, and renders below the
        # company name (ExperienceRoleTitle already renders Experience.title
        # bold, so no template change is needed for that part). Product name
        # is left off for now -- just the title alone.
